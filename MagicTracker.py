...
class WandTrail:
    ...
    def draw(self, frame):
        # 1. วาดเส้น Trail — draw directly without frame.copy() per segment
        pts_list = list(self.trail)
        if len(pts_list) >= 2:
            for i in range(1, len(pts_list)):
                p, pp = pts_list[i], pts_list[i - 1]
                alpha = p['life']
                # Bake alpha into color brightness instead of addWeighted
                color = (int(255 * alpha), int(150 * alpha * alpha), int(200 * alpha))
                thickness = max(1, int(alpha * 5))
                cv2.line(frame, (pp['x'], pp['y']), (p['x'], p['y']), color, thickness, cv2.LINE_AA)
...
while True:
    ...
    if state == "IDLE":
        ...
        if not is_holding:
            ...
            release_counter = 0  # reset if hand goes still again

        # Only transition after wand has been moving for 6+ frames (not just 1 jitter frame)
        if release_counter >= 6:
            # Go straight to DRAW and start recording from right here
            pts.clear()
            if px is not None:
                pts.append((px, py))
        ...
        if len(valid_pts) > 1:
            # OPT: math.hypot instead of np.linalg.norm for 2D distance
            dx = valid_pts[-1][0] - valid_pts[0][0]
            dy = valid_pts[-1][1] - valid_pts[0][1]
        ...
        if is_holding and elapsed > 0.8:
            ...
            state, hold_counter, release_counter, pts = "IDLE", 0, 0, deque(maxlen=64)
            history.clear()
